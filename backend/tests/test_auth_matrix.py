...
async def test_11_non_admin_creates_project(client, seed):
    ...
    bob_can = await client.post("/projects", headers=bearer("bob"),
...
async def test_12_admin_creates_project(client, seed):
    org = await seed.org(admin="alice")
    r = await client.post("/projects", headers=bearer("alice"),
                          json={"org_id": org, "name": "Web2"})
...
async def test_13_role_revoked_invalidates_cache(client, seed):
    ...
    assert (await client.patch(f"/tasks/{task}", headers=bearer("bob"),
    ...
    demote = await client.patch(f"/projects/{proj}/members/{bob_uid}",
                                headers=bearer("alice"), json={"role": "viewer"})
