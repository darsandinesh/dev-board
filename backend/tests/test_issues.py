...
async def test_comments_require_view_access(client, seed):
    ...
    assert (await client.get(f"/tasks/{tid}/comments", headers=bearer("carol"))).status_code == 403
    assert (
        await client.post(f"/tasks/{tid}/comments", headers=bearer("carol"), json={"body": "hi"})
    ).status_code == 403
...
async def test_issue_keys_and_seq(client, seed):
    ...
    t1 = await client.post(
        "/tasks", headers=bearer("alice"), json={"project_id": proj, "title": "one"}
    )
    t2 = await client.post(
        "/tasks", headers=bearer("alice"), json={"project_id": proj, "title": "two"}
    )
...
async def test_issue_links(client, seed):
    ...
    r = await client.post(
        f"/tasks/{a}/links", headers=bearer("alice"), json={"target_id": b, "link_type": "blocks"}
    )
    assert r.status_code == 201
    links = await client.get(f"/tasks/{a}/links", headers=bearer("alice"))
    assert any(lk["link_type"] == "blocks" and lk["target_id"] == b for lk in links.json())
    # self-link rejected
...
async def test_mention_notifies(client, seed):
    ...
    tid = await seed.task(proj, editor="alice")
    await client.post(
        f"/tasks/{tid}/comments", headers=bearer("alice"), json={"body": "hey @bob please look"}
    )
    notes = (await client.get("/notifications", headers=bearer("bob"))).json()
...
async def test_assignee_can_comment(client, seed):
    ...
    tid = created.json()["id"]
    r = await client.post(f"/tasks/{tid}/comments", headers=bearer("carol"), json={"body": "on it"})
    assert r.status_code == 201
