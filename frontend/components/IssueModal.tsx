...
import { CalendarDays, ChevronRight, Paperclip, Send, X } from "lucide-react";
...
/** Sidebar detail row: label on top, control below (Jira-style). */
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
            ...
            {!ro && (
              <div className="mt-2 flex gap-2">
                <select value={linkType} onChange={(e) => setLinkType(e.target.value as LinkType)}
                  className="rounded-md border px-2 py-1.5 text-xs text-slate-600">
                ...
                <select value={linkTarget} onChange={(e) => setLinkTarget(e.target.value)}
                  className="flex-1 rounded-md border px-2 py-1.5 text-xs text-slate-600">
          ...
          {/* Activity: Comments | History tabs */}
          <div>
            ...
            {tab === "comments" ? (
              ...
              <ul className="space-y-1.5 border-l-2 border-slate-100 pl-3">
                ...
                {(!activity || activity.length === 0) && (
                  <li className="text-xs text-slate-400">No activity.</li>
                )}
              </ul>
            )}
          </div>
        ...
        {/* SIDEBAR — Details */}
        <aside className="w-72 shrink-0 space-y-4 overflow-y-auto border-l bg-slate-50/50 p-5">
          <select
            value={task.status}
            disabled={ro}
            ...
            <Field label="Labels">
              ...
              {task.labels.length > 0 && (
                <div className="mt-1 flex flex-wrap gap-1 px-2">
                  {task.labels.map((l) => (
                    <span key={l} className="rounded bg-slate-100 px-1.5 py-0.5 text-[11px] text-slate-500">{l}</span>
                  ))}
                </div>
              )}
            </Field>
